...
function BadgePill({ badge }: { badge?: string }) {
  ...
  return (
    <div className="absolute left-3 top-3 z-10">
      <span className="rounded-full bg-foreground px-3 py-1 text-[10px] font-semibold uppercase tracking-wider text-white shadow-sm">
        {badge}
      </span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 px-3 pb-4 pt-10 text-center sm:px-4 sm:pb-5">
        ...
        {subtitle ? (
          <p className="mt-1 text-[11px] leading-snug text-white/85 transition-all duration-300 group-hover:-translate-y-1 group-hover:opacity-0 sm:text-xs">
            {subtitle}
          </p>
        ) : null}
      </div>
